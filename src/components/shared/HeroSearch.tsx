        ...
        <div className="mx-auto mt-6 max-w-3xl sm:mt-8">
          <div className="rounded-2xl bg-white p-2 shadow-2xl shadow-black/30 sm:p-2.5">
            <div className="flex flex-col gap-2 sm:flex-row sm:items-stretch">
              <div className="flex flex-1 items-start gap-2 rounded-xl bg-slate-50 px-3 py-2.5">
                <MapPin className="h-5 w-5 shrink-0 text-emerald-500 mt-0.5" />
                <div className="min-w-0 flex-1"><span className="block text-[10px] font-bold uppercase tracking-wider text-slate-400">Municipio de recogida</span>
                  <select value={city} onChange={e => setCity(e.target.value)} className="w-full cursor-pointer bg-transparent text-sm font-semibold text-slate-900 focus:outline-none">
                    <option value="">Todos los municipios</option>{DEPARTURE_CITIES.map(c => <option key={c} value={c}>{c}</option>)}
                  </select>
                  {city && (
                    <p className="mt-1.5 text-[10px] leading-tight text-emerald-700/80">
                      💡 Cada viaje tendrá puntos de recogida en distintas localidades del municipio (cabecera, rancherías, etc.). Revisa la dirección exacta en la ficha del viaje.
                    </p>
                  )}
                </div>
              </div>
